...
const TaskContext = createContext<{
  ...
  perLoad: number
  getTaskByPagination: (page: number) => boolean
}>({
  tasks: [],
  ...
  perLoad: 8,
  getTaskByPagination: (page: number) => false,
})
export const TaskProvider: React.FC = (props) => {
  ...
  const getTaskByPagination = async(page: number) => {
    await getAllTasks.run(perLoad, page * perLoad)
    if()
  }
